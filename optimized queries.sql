...
-- Q17
 ...
 where
         ...
         and lineitem.l_partkey = AvgCalc.l_partkey
         and p_brand = 'Brand#52'
         and p_container = 'JUMBO CAN'
         and l_quantity < AvgCalc.avg;
...
half_quantities as (
    select l_partkey, l_suppkey, 0.5 * SUM(l_quantity) as half_1994_orders
    from lineitem
    where l_shipdate >= date ('1994-01-01') 
    and l_shipdate < date ('1995-01-01')
    group by l_partkey, l_suppkey
),
large_lime_part_supplier as (
        select ps_suppkey, ps_partkey
        from partsupp
        join half_quantities on partsupp.ps_partkey = half_quantities.l_partkey
                          and partsupp.ps_suppkey = half_quantities.l_suppkey
        where ps_partkey in (select p_partkey from lime_part)
        and ps_availqty > half_quantities.half_1994_orders
)
 select
         s_name,
         s_address
 ...
 where  
         supplier.s_suppkey in (select ps_suppkey from large_lime_part_supplier)
         and n_name = 'VIETNAM'
 order by
         s_name;
         ...
         (
                 ...
                 where
                         substr(c_phone, 1, 2) in
                                 ('41', '28', '39', '21', '24', '29', '44')
                         and c_acctbal > (Select avg_bal from Average_balance)
